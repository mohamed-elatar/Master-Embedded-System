...
typedef struct
{
	EXTI_GPIO_Mapping_t EXTI_Pin;		// mapping external interrupt based on @ref EXTI_def_Pin
	uint8_t				Trigger_Case;	// select (failing | raising | both) based on @ref EXTI_Trigger_Pin
	uint8_t				IRQ_EN;			// enable or disable mask IQR based on @ref EXTI_IRQ_EN
	void (* PF_IRQ_CallBack)(void);		// pointer to function , it executed when the interrupt occurred in  main

}EXTI_Pin_Configure_t;
...
//@ref EXTI_Trigger_Pin
#define EXTI_Trigger_Raising					0
#define EXTI_Trigger_Failing					1
#define EXTI_Trigger_Raising_And_Failing		2
